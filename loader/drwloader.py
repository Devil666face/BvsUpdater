...
class DRWLoader:
    ...
    def __init__(
        ...
        logger.info(f"Start for download {path_to_folder}")
        if self.load():
            logger.success(f"{self.path_to_folder} successfull load")
            self.make_finally_zip_archive(zip_name=path_to_folder)
        else:
