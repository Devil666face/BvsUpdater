...
class WeeklyPacker(Renamer):
    ...
    def __init__(
        ...
        logger.debug(f"Find folder with name {self.date_old}")
    ...
    def is_name_equal(self, new_name: str, old_name: str):
        ...
        if new_product_name == old_product_name:
            logger.success(
                f"Find equal name success old:{old_product_name} new:{new_product_name}"
            )
            return True
        return False
...
class WeeklyCompair(WeeklyPacker):
    ...
    def __init__(
        ...
        self.rsync()
        if self.is_unique():
            logger.success(f"Archives is unique")
            (
    ...
    def rsync(self):
        ...
        # command = f"rsync -acvin --compare-dest={self.old_dir_for_unpack} {self.new_dir_for_unpack}/ {self.diff_dir_for_unpack} > {self.diff_file}"
